...
It inspected the scene, generated a structured patch, then changed material, lighting, camera, and environment in the browser.
...
它先检查当前场景，再生成结构化 patch，最后在浏览器里真实修改材质、灯光、相机和环境。
...
4. If `OPENROUTER_API_KEY` is set, type a custom prompt and click `Run GLM agent`.
